...
需要通行密钥时，在 Worker 的「设置 → 变量和机密」中添加运行时变量 `PUBLIC_URL`（如 `https://notes.example.com`），不要只设置构建环境变量。R2、KV 部署配置均启用了 `keep_vars = true`，Git 推送触发的后续部署会保留配置文件未声明的控制台变量；`[vars]` 中显式声明的同名变量仍以文件值为准。若之前的部署已删除 `PUBLIC_URL`，需重新添加一次。
